...
- Interactive World Map: AI-generated parchment-style region map (via Gemini) stored in `world_state.mapImageData`; map prompt includes story terrain hints and region names; location coordinates in `world_state.mapCoords` (0-100 range); coordinates use region-aware clustering (`assignAllLocationCoords`): same-region locations cluster together, same-name-base locations (e.g. "Modest Town Tavern" + "Modest Town Market") cluster tightly (2-5 units); inter-cluster distance uses `firstVisitedTurn` gaps × 1.5 (capped at 30) as travel distance proxy — locations visited many turns apart are placed farther on the map; coordinates are stable (existing coords preserved when new locations added); canvas-based renderer with fog-of-war (offscreen canvas compositing — fog covers unvisited areas, visited locations revealed), location type icons (canvas-drawn symbols for town/tavern/market/forest/water/cave/etc.), glowing current location pin, dotted travel paths, zoom/pan, click-to-travel; collapsible map legend (? button, bottom-right) shows only location types present in current data plus danger skull and current location indicators; `isTrueThreat()` filters false positives from GM narrative notes in `threat` field; map regeneration via UI button (`data-testid="regenerate-map"`) and `?regenerate_map=1` API param; fullscreen overlay mode; list view fallback toggle; `GET /api/parties/:id/map` returns map image + location data; `?recalculate=1` forces coord recalc; `client/src/components/WorldMap.tsx` is the interactive canvas component
