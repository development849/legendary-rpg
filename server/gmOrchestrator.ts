...
CRITICAL — A MISSED ATTACK ROLL STILL ENDS THAT CHARACTER'S TURN:
When a [ROLL RESULT] for an attack roll is a MISS (FAILURE), the character's attack action is FULLY RESOLVED. You MUST NOT request another attack roll for the same character in this response. A miss is NOT a "try again" — it consumes the action just like a hit does. The round must continue: ${campaign.npcControl === "player" ? "request attack rolls for any player-controlled companions, then" : "resolve any GM-controlled companion attacks, then"} narrate and resolve ALL surviving enemy counterattacks (with HP_CHANGED on hits), THEN set turn_hint back to the player for the next round. Repeatedly asking the same character to swing again on a miss is a CRITICAL ERROR — it traps the player in solo combat with no enemy or companion action.

CRITICAL — AFTER PLAYER ATTACK RESOLVES (HIT OR MISS), THE ROUND CONTINUES IN THE SAME RESPONSE:
The trigger for the rest of the combat round is the PLAYER'S ATTACK ROLL resolving — whether it was a HIT or a MISS. On a HIT, request the damage roll first; once the damage roll comes back, continue the round. On a MISS, continue the round immediately. "Continue the round" means: ${campaign.npcControl === "player" ? "(a) emit a fresh dice_requests entry for EACH player-controlled companion's attack this round (using their weapon stats from the NPC log), and (b) " : "(a) resolve every GM-controlled companion's attack in the narrative with the [rolls X + Y = Z vs AC N] format and HP_CHANGED on hits, and (b) "}narrate and resolve EVERY surviving enemy's counterattack with the [rolls X + Y = Z vs AC N] format and HP_CHANGED on hits, then (c) set turn_hint back to the player for round 2. A response that ends after a player attack roll/damage roll without companion and enemy actions is WRONG.
...
- DUAL WIELDING: When a character has TWO one-handed weapons equipped (look for two [EQUIPPED] weapons without two_handed), they are dual wielding. The dual-wield bonus attack is part of THE SAME ROUND STEP as their main attack — it does NOT delay the rest of the round. After the main attack roll resolves (and damage roll if it hit), immediately request the off-hand attack roll in the SAME response. Once both attacks have fully resolved (or both have missed), continue the round normally with companions and enemies. Do NOT let dual wielding skip enemy/companion turns. The off-hand attack uses d20 + weapon bonus for the attack roll, and the weapon's damage die for damage but with NO ability modifier added (just the weapon bonus if any). Example: character has Longsword [EQUIPPED] and Dagger [EQUIPPED] → main attack with Longsword, then bonus attack with Dagger, then companion turns, then enemy turns.
- TWO-HANDED WEAPONS: A character with a two_handed weapon gets one attack per combat round but with the larger damage die.
- COMPANION ATTACKS: ${campaign.npcControl === "player" ? "Players control their companions in combat. EVERY combat round, after the player's attack roll resolves (hit OR miss, and after any damage roll), you MUST emit a fresh dice_requests entry for EACH player-controlled companion's action this round — typically an attack roll using the companion's equipped weapon (look up their weapon damage die and attack bonus from the ACTIVE NPC COMPANIONS section above). Example dice_requests entry: {\"character\": \"Mira\", \"die\": \"d20\", \"modifier\": 2, \"advantage\": \"normal\", \"purpose\": \"Attack roll with Bone Dagger\"}. Narrating that a companion is 'preparing to strike', 'stepping in', 'ready to act', or 'tightening her grip' is NOT a turn — only an actual dice_requests entry counts. If you skip the companion's roll request in any combat round, the player has no way to act for them and combat stalls. Companions still have their own personality — narrate their reactions and dialogue — but the player decides and rolls their tactical actions." : "In combat, companions attack on their own initiative. Roll their attacks and damage yourself (narrate it with the [rolls X + Y = Z vs AC N] format) and apply HP_CHANGED to enemies on hits — don't ask the player to roll for NPCs."}

ENEMY ATTACKS — MANDATORY:
Combat is a two-way exchange. Enemies FIGHT BACK. This is the most important combat rule:
- COMBAT ROUND STRUCTURE — every round, in this exact order, all in the SAME GM response:
  (1) PLAYER'S ATTACK: resolve the player's attack roll. On a HIT → request the damage roll, then on the next response when damage comes back, narrate impact and emit HP_CHANGED on the target. On a MISS → narrate the miss. Either way, the player's action is now done.
  (2) ${campaign.npcControl === "player" ? "PLAYER-CONTROLLED COMPANIONS: for EACH living companion in ACTIVE NPC COMPANIONS, emit a dice_requests entry for their attack this round (d20 + their weapon bonus, purpose mentioning their weapon). Do NOT just narrate them as 'ready to strike' — always emit the roll request." : "GM-CONTROLLED COMPANIONS: for EACH living companion, narrate their attack with the [rolls X + Y = Z vs AC target_AC] format and emit HP_CHANGED on the enemy if they hit."}
  (3) ENEMIES: for EACH surviving enemy, narrate their attack against a party member (player or companion) with the [rolls X + Y = Z vs AC N] format. On hits, emit HP_CHANGED with a NEGATIVE delta on the targeted character. On misses, narrate the whiff with the same bracket math.
  (4) TURN HINT: set turn_hint back to the player character for the next round.
  You MUST NOT give the player another attack roll until steps (2) and (3) have been completed in the same response.
- ENEMY TURNS: After the player's attack is resolved (HIT or MISS, and after any damage roll), ALL surviving enemies MUST attack in the SAME response. You MUST narrate each enemy's counterattack and resolve it yourself — roll their attack (decide the result: hit or miss based on d20 + their attack bonus vs the player's AC from their character sheet) and if they hit, emit HP_CHANGED with a NEGATIVE delta against the targeted character. Do NOT skip enemy turns. Do NOT let the player attack repeatedly without enemies responding. Do NOT end your response without resolving enemy attacks when combat is active.
...
- ONGOING COMBAT: Combat continues in rounds until enemies are defeated, flee, or surrender. Each round follows the COMBAT ROUND STRUCTURE above: player → companions → enemies → turn_hint. Don't end combat early by having enemies vanish or give up after one hit unless narratively justified. Make fights last 2-4 rounds minimum for meaningful encounters. NEVER give the player two consecutive attack opportunities without enemies (and companions, when applicable) acting between them — that means a turn was skipped, which is a critical error. Companion and enemy steps only fire while combat is ACTIVE and at least one able hostile remains; if the player's attack drops the last enemy, skip straight to a victory beat (no companion roll request, no enemy counterattack).
- INCAPACITATED & OFF-SCENE COMBATANTS: Only LIVING and ABLE combatants who are present in the current combat take a turn. Skip companions/enemies who are at 0 HP, unconscious, paralysed, fled, or otherwise unable to act. Skip companions who are off-scene or have been split from the party — do not request rolls for them.
...
NPCs can join the party or leave based on story events. Use NPC_JOINED_PARTY when an NPC decides to travel with, help, or fight alongside the party (e.g., they strike a deal, swear an oath, are rescued and pledge aid, or choose to join of their own accord). Use NPC_LEFT_PARTY when a companion departs — they've fulfilled their purpose, been killed, betrayed the party, or gone their own way. Active companions listed in ACTIVE NPC COMPANIONS above travel with the party. Include them naturally in scenes: they react, comment, assist in combat, and interact with the world. ${campaign.npcControl === "player" ? "TACTICAL CONTROL is in PLAYER mode for this campaign — the player decides and rolls each companion's combat actions (see COMPANION ATTACKS rule above). You still speak for companions out of combat (dialogue, personality, autonomous reactions to the world), but in combat you must request dice for their actions instead of resolving them yourself." : "TACTICAL CONTROL is in GM mode for this campaign — companions are NOT player-controlled, you speak and act for them in combat and out."} When a companion acts meaningfully, briefly narrate their action alongside the main narrative. Companions can have their own agendas, secrets, and moments — use them for drama and flavor. If a companion joins or leaves, emit the appropriate update AND weave their departure/arrival into the narrative naturally.
...
export async function runGM(
  ...
  // bookkeeping (NPC_MET, NPC_RELATIONSHIP_CHANGED, LOCATION_MENTIONED).
  // We strip those types from the narrator's updates and replace with the
  // chronicler's. PLOT_FACT_SET is additive — narrator + chronicler facts are
  // merged with the chronicler winning on key conflicts.
  try {
    const prevState: any = worldSnap?.state ?? {};
    const knownLocationsForChronicler = (prevState.locations ?? []).map((l: any) => ({
      name: l.name, region: l.region, rumored: l.rumored === true,
    }));
    const chroniclerResult = await runChronicler({
      playerIntent: ctx.playerIntent ?? "",
      narrative: cleanNarrative,
      previousNarrative: prevGmMsg?.content ?? null,
      knownNpcs: npcs.map((n: any) => ({ name: n.name, role: n.role })),
      knownPlayerChars: chars.map((c: any) => ({ name: c.name })),
    ...
    // actually succeeded. On any failure (LLM error, timeout, schema failure,
    // partial salvage) we KEEP the narrator's original ledger intact — better to
    // accept the narrator's NPC_MET than to silently drop bookkeeping for the turn.
    if (chroniclerResult.ok) {
      const chroniclerOwnedTypes = new Set(["NPC_MET", "NPC_RELATIONSHIP_CHANGED", "LOCATION_MENTIONED"]);
      const narratorRest: any[] = [];
      const narratorFactsByKey = new Map<string, any>();
      for (const u of updates) {
        if (chroniclerOwnedTypes.has(u.type)) continue;
        if (u.type === "PLOT_FACT_SET") {
          const k = String(u.key ?? "").trim().toLowerCase().replace(/\s+/g, "_");
          if (k) narratorFactsByKey.set(k, u);
          continue;
        }
        narratorRest.push(u);
      }
      // Chronicler facts override narrator facts on key conflict.
      for (const cu of chroniclerResult.updates) {
        if (cu.type === "PLOT_FACT_SET") {
          const k = cu.key.trim().toLowerCase().replace(/\s+/g, "_");
          if (k) narratorFactsByKey.set(k, cu);
        }
      }

      // Splice replacement: narrator-only updates + dedup'd facts + chronicler entity updates.
      const rebuilt: any[] = [...narratorRest, ...narratorFactsByKey.values()];
      for (const cu of chroniclerResult.updates) {
        if (cu.type !== "PLOT_FACT_SET") rebuilt.push(cu);
      }
      updates.length = 0;
      updates.push(...rebuilt);

      console.log(`[Chronicler] OK — emitted ${chroniclerResult.updates.length} update(s): ${chroniclerResult.updates.map(u => `${u.type}${"name" in u ? `(${(u as any).name})` : "key" in u ? `(${(u as any).key})` : ""}`).join(", ") || "none"}`);
    } else {
      console.warn(`[Chronicler] FAILED (${chroniclerResult.reason}) — keeping narrator's ${updates.length} update(s) as fallback.`);
    }
  } catch (chroniclerErr) {
    console.error("[Chronicler] Pass failed, keeping narrator updates as-is:", chroniclerErr);
  }
  ...
  if (scene?.location) {
    ...
    if (!existing) {
      ...
      const wasRumored = existing.rumored === true;
      ...
      // Promote a rumored location to visited the first time the party actually arrives.
      if (wasRumored) {
        existing.rumored = false;
        existing.firstVisitedTurn = turnNum;
        delete existing.mentionedTurn;
        delete existing.source;
        console.log(`[GM] Rumored location promoted to visited: "${existing.name}"`);
      }
      if (titleChanged || wasRumored) {
        generateLocationBackground(
          ctx.partyId,
          scene.location,
          sceneTitle,
  ...
  for (const update of updates) {
    try {
      switch (update.type) {
        ...
        case "LOCATION_MENTIONED": {
          // A place that was named/referenced but the party did NOT visit this turn.
          // Stored alongside visited locations with `rumored: true` so the journey
          // map can render it with a "rumored" badge and no thumbnail.
          const locName = (update.name ?? "").trim();
          if (!locName) break;
          const [currentSnap] = await db.select().from(worldState).where(eq(worldState.partyId, partyId));
          const currentState: any = currentSnap?.state ?? {};
          const locations: any[] = currentState.locations ?? [];
          ...
          const updatedState = { ...currentState, locations };
          await db.insert(worldState)
            .values({
              partyId,
              state: updatedState,
          ...
          console.log(`[GM] Location mentioned (rumored): "${locName}" in region "${update.region ?? "Unknown"}"`);
          break;
        }
      }
    } catch (err) {
